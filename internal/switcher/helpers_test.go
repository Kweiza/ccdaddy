...
func seed(t *testing.T, uuid, email string) store.Account {
	...
	a := store.Account{Provider: provider.Claude, SubscriptionStatus: "active", UUID: uuid, Email: email}
...
func seedSeat(t *testing.T, uuid, email, seatTier string) store.Account {
	...
	a := store.Account{Provider: provider.Claude, SubscriptionStatus: "active", UUID: uuid, Email: email, SeatTier: seatTier}
	if err := s.Add(a, oauthBlob("RT-"+uuid)); err != nil {
		t.Fatal(err)
	}
	got, ok := s.Get(uuid)
