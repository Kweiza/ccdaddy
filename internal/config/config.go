// Package config is ~/.ccdad/config.toml: the §7 knobs, their defaults, and the
// re-read §8.4's tick loop performs.
//
// KEY STYLE, and this is a compatibility commitment rather than a preference.
// Spec §7 names its knobs inconsistently — `hysteresis_pct` and
// `max_auto_spend` in snake_case, `credit.maxAutoSpend` in camel. Every key
// here is SNAKE_CASE, which is the style accounts.toml already uses
// (`active_uuid`) and the style §7's own TOML examples are written in; the one
// camelCase spelling loses. Section grouping follows the spec: `credit` keeps
// its own table, so the spec's `credit.maxAutoSpend` is this file's
// `credit.max_auto_spend`.
//
...
type Config struct {
	// Threshold is §7.1's utilization percent above which an account counts as
	// spent.
	Threshold float64
	// HysteresisPct is §7.2's additive margin, in points of headroom.
	HysteresisPct float64
	// HeadroomRatio is §7.2's multiplicative margin on the same axis.
	HeadroomRatio float64
	// Cooldown is §7.2's minimum gap between two switches.
	Cooldown time.Duration
	// RecoveryHysteresis is §7.2's margin on the recovery axis.
	RecoveryHysteresis time.Duration
	// Strategy is which question the ranking asks.
	Strategy strategy.Strategy
	// MaxAutoSpend is §7.3's ceiling, in the currency's major unit. 0 refuses,
	// and is the default.
...
//
// Horizon is deliberately left at its zero value: §7 names no key for it, so
// strategy.DefaultRecoveryHorizon stays the single place it is decided.
...
// mechanism off, which §7.2 offers no way to do. Pointers make both cases
// decidable, so absence takes the default and an explicit 0 is validated like
// any other value.
...
//
// It is the whole loader minus the filesystem, which is what lets every trap in
// §7.3 be pinned by a test that writes no file. A document that fails
// validation yields the ZERO Config alongside its error and never a partly
// applied one: half a config file is not a configuration, and the caller that
// falls back to the defaults has to be sure it is falling back to all of them.
...
// older one running — the same rule §4.2 states for the credentials file — so
// the loader ignores them and this reports them, which is how `ccdad config
// list` can say "these are being ignored" instead of the file silently doing
// less than it looks like it does. It is also the only warning a hand-edited
// typo ever gets, since a typo is indistinguishable from a future key.
...
// Reloader is §8.4's "pick up external config changes", with the property that
// paragraph does not state and the daemon depends on: a broken edit must not
// stop the engine.
//
...
func applyDuration(dst *time.Duration, v *string, key string) error {
	...
	if d <= 0 {
		return fmt.Errorf("%s in %s: %q is not positive, and §7.2 offers no way to switch an anti-flap mechanism off", key, FileName, *v)
	}
	*dst = d
	return nil
}
...
// it. §7.3 spells this out for max_auto_spend, and it holds for every float
// here: a NaN threshold makes `100-pct > threshold` false for every account, so
// nothing is ever over threshold and the recovery mode becomes unreachable.
...
func validHeadroomRatio(v float64) error {
	...
	// "margin", which inverts §7.2's mechanism instead of loosening it. 1.0 is
	// the honest way to say "no multiplicative margin".
	if v < 1 {
