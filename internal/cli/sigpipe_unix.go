...
// EPIPE branch is unreachable dead code. Spec §9.3 requires the opposite —
// "EPIPE is not an error. `ccdad list --json | head -1` exits 0" — and that
// held only by accident, for outputs small enough to fit the pipe buffer.
//
