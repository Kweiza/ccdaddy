...
//
// It bounds the work as well, and that is why it is not larger. The search runs
// one simulation per candidate count and a simulation costs time proportional to
// the square of the fleet it is handed: measured on this tree, one run over a
// fleet of 256 accounts carrying five windows each takes some 30 ms, and the
// worst case a six-account fleet can actually reach -- every account burning a
// whole weekly window inside the four-hour measured span, which is the fastest
// rate six accounts can produce -- searches to 255 and takes 4 s. An ordinary
// fleet answers in single-digit milliseconds, because the search stops at the
// first count that holds.
const maxNeeded = 256
...
func needFor(sim []simAccount, rates map[usage.WindowName]float64, five, weekly, both []usage.WindowName, now time.Time, basis bool) need {
	...
	out := need{has: true}
	if holds(len(seats), both) {
		// The same search downward, so a fleet that holds learns how much slack
		// it has and not merely that it has some. It stops at one: a fleet of
		// no accounts runs dry because there is nobody to be live, which is an
		// artefact of having nobody rather than anything the rate did.
		out.count = len(seats)
		for out.count > 1 && holds(out.count-1, both) {
			out.count--
		}
	} else {
		// Upward, one seat at a time, to the first count that survives the
		// horizon. Reaching the ceiling without one is reported as the ceiling
		// and flagged, so nothing downstream can read the bound as a count
		// somebody could go and buy.
		out.count, out.capped = ceiling, true
		for n := len(seats) + 1; n <= ceiling; n++ {
			if holds(n, both) {
				out.count, out.capped = n, false
				break
			}
		}
	}
