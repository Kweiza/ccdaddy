...
func TestTheCursorStopsAtTheEndsRatherThanWrapping(t *testing.T) {
	// Walked to the top first: this picker opens on the value already in force,
	// which is not the first row, so "up from the top" has to be arranged rather
	// than assumed.
	p := strategyPicker("headroom", UnicodeGlyphs).Move(-len(strategyPicker("headroom", UnicodeGlyphs).items))
	if p.cursor != 0 {
		t.Fatalf("walking up landed on %d, want the first row", p.cursor)
	}
...
func TestAPickerOverAnEmptyStoreChoosesNothingAndSaysSo(t *testing.T) {
	p := picker{title: "Nothing to choose", glyphs: UnicodeGlyphs}
...
func TestThePickerNeverExceedsTheWidthItWasGiven(t *testing.T) {
	...
	for _, width := range []int{20, 40, 60} {
		for i, line := range strings.Split(strategyPicker("headroom", UnicodeGlyphs).Body(width, theme.Of(theme.Dark)), "\n") {
			if got := ansi.StringWidth(line); got > width {
				t.Errorf("at width %d line %d is %d columns: %q", width, i, got, line)
			}
		}
