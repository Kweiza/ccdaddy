...
- **Work in a worktree. Always, and BEFORE the first file is modified.**

  Call `EnterWorktree` at the start of any session that will change a file —
  before the first edit, not after the first few. Read-only exploration needs no
  worktree; the moment the answer is "I am going to change something", the
...
- **Never add AI attribution to a git object.** No `Co-Authored-By: Claude`, no
  "Generated with Claude Code", no robot marker, in commit messages or PR
  descriptions. The commit log stays clean. `.git/hooks/commit-msg` enforces
  this and routing the message through a file or an editor does not get past it.
